...
| httpd.MaxTotalMemory | no | (5.5+) Specifies the maximum total physical memory (of MBytes) that all worker processes can consume before the pool starts to recyle worker processes. It ranges from `256` to 80% of maximum physical memory installed on the machine. The default is `0`, where Jexus automatically adjusts the usage. Note that for each worker process the minimum physical memory usage is 128-MBytes. |
